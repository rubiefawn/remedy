...
* Rolling triumph on any of these checks restores `1 💖` and ends the condition; rolling disaster renders the creature Unconscious.
...
* A Crippled creature suffers disaster on all Strength, Agility, and Vitality-based checks on a natural result of 1–2, and takes `1d4 🩸` for every level of this condition when they suffer disaster because of it.

{% hint style="warning" %}
The Crippled condition can be gained more than once. If a creature has more than one level of Crippled, the range for disaster increases by 1 for each level \(e.g. 4 levels of crippled triggers disaster on a natural result of 1–5\).
{% endhint %}
...
* A Frightened creature suffers different effects depending on the margin of failure on its Resist Fear check:
  * `1–3`: Fight. The Frightened creature must attempt to engage the source of the condition, making all attack and spell rolls with threat.
  * `4–6`: Flight. The Frightened creature must attempt to flee from the source of the condition, and is unable to willingly move closer to the source. In addition, the Frightened creature must succeed on a difficulty 5 Resist Fear check or become Traumatized.
...
* A Sick creature takes `1d4 🦠` after every short or long rest.
...
* A Wounded creature suffers disaster on all rolls requiring the use of that body part on a natural result of 1–2.
